...
void GOL::tick() const
{
    ...
    for (int x = 0; x < gridWidth / cellSize; x++)
    {
        for (int y = 0; y < gridHeight / cellSize; y++)
        {
            ...
            if (y > 0 && grid[x * (gridHeight / cellSize) + (y - 1)].active)
                aliveNeighbors++; // Check if top is alive
            // Check if top right cell exists
            if ((x < gridWidth / cellSize - 1 || y > 0) && grid[(x + 1) * (gridHeight / cellSize) + (y - 1)].active)
                aliveNeighbors++; // Check if top right cell is alive
            // Check if right cell exists
            if ((x < gridWidth / cellSize - 1) && grid[(x + 1) * (gridHeight / cellSize) + y].active)
                aliveNeighbors++; // Check if right is alive
            // Check if bottom right cell exists
            if ((y < gridHeight / cellSize - 1 || x < gridWidth / cellSize - 1) && grid[(x + 1) * (gridHeight / cellSize) + (y + 1)].active)
                aliveNeighbors++; // Check if bottom right is alive
            // Check if bottom cell exists
            if ((y < gridHeight / cellSize - 1) && grid[x * (gridHeight / cellSize) + (y + 1)].active)
                aliveNeighbors++; // Check if bottom is alive
            // Check if bottom left cell exists
            if ((x > 0 || y < gridHeight / cellSize - 1) && grid[(x - 1) * (gridHeight / cellSize) + (y + 1)].active)
                aliveNeighbors++; // Check if bottom left is alive
            // Check if left cell exists
            if (x > 0 && grid[(x - 1) * (gridHeight / cellSize) + y].active)
                aliveNeighbors++; // Check if left is alive
            // Check if top left cell exists
            if ((x > 0 || y > 0) && grid[(x - 1) * (gridHeight / cellSize) + (y - 1)].active)
                aliveNeighbors++; // Check if top left is alive
